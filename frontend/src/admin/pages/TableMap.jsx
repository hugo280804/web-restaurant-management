 ...
 useEffect(() => {
  ...
  fetch("http://localhost:5000/api/floors")
    ...
    .then(data => {
      ...
      const firstFloor = data[0]?.floor_ID || "";
      setCurrentFloor(firstFloor);

      // 👉 FIX: set luôn vào form
      setForm(prev => ({
        ...prev,
        floor_ID: firstFloor
      }));
  ...
  fetch("http://localhost:5000/api/tables")
    ...
    .then(data => {
      const mapped = data.map(t => ({
        ...
        floor_ID: Number(t.floor_ID), // 👉 FIX kiểu
        x: t.pos_x || 100,
        y: t.pos_y || 100,
      }));
      setTables(mapped);
  ...
  const floorId = form.floor_ID || currentFloor;
